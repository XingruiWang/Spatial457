...
def delete_object(obj):
    """ Delete a specified Blender object (Blender 2.8+) """
    ...
    # Set it as active (sometimes needed)
...
def add_object_with_texture(object_dir, name, obj_pth, color_name, scale, loc, theta=0):
    """
    ...
    # find obj path
    cate_name = obj_pth.split('/')[0]
    obj_name = name
    obj_pth = os.path.join(object_dir, cate_name, obj_name, color_name, 'object.obj')
...
def set_dome_texture(ground_name, texture_path):
    my_object_blender = bpy.data.objects[ground_name]

    # my_object_blender.cycles_visibility.shadow = False
    # my_object_blender.cycles_visibility.diffuse = False
    new_material = bpy.data.materials.new(name="New_Material")
    my_object_blender.data.materials.append(new_material)
    material = my_object_blender.data.materials[0]
    ...
    if principled_bsdf_node is None:
        principled_bsdf_node = texture_node.new(type='ShaderNodeBsdfPrincipled')
...
def modify_read_color(current_obj, material_name, shape, color, texture, mat_freq):
...
def _set_ambient_light_hdri(hdri_filepath=None, hdri_rotation=(0.0, 0.0, 0.0), strength=1.0):
    # Ensure the HDRI node is connected
    ...
    ambient_node = tree.nodes.new(type="ShaderNodeBackground")
    ambient_node.inputs["Color"].default_value = (0.0, 0.0, 0.0, 1.0)
    ambient_node.location = (700, 0)

    bg_node = tree.nodes.new(type="ShaderNodeBackground")
    bg_node.inputs["Color"].default_value = (0.0, 0.0, 0.0, 1.0)
    bg_node.location = (700, -120)

    # Link the nodes
    links.new(lightpath_node.outputs["Is Camera Ray"], mix_node.inputs["Fac"])
    links.new(ambient_node.outputs["Background"], mix_node.inputs[1])
    links.new(bg_node.outputs["Background"], mix_node.inputs[2])
    links.new(mix_node.outputs["Shader"], out_node.inputs["Surface"])

    # Create nodes for HDRI images, initially disconnected
    coord_node = tree.nodes.new(type="ShaderNodeTexCoord")

    bg_mapping_node = tree.nodes.new(type="ShaderNodeMapping")
    bg_mapping_node.location = (200, 200)

    bg_hdri_node = tree.nodes.new(type="ShaderNodeTexEnvironment")
    bg_hdri_node.location = (400, 200)

    links.new(coord_node.outputs["Generated"], bg_mapping_node.inputs["Vector"])
    links.new(bg_mapping_node.outputs["Vector"], bg_hdri_node.inputs["Vector"])

    illum_mapping_node = tree.nodes.new(type="ShaderNodeMapping")
    illum_mapping_node.location = (200, -200)

    ambient_hdri_node = tree.nodes.new(type="ShaderNodeTexEnvironment")
    ambient_hdri_node.location = (400, -200)

    links.new(coord_node.outputs["Generated"], illum_mapping_node.inputs["Vector"])
    links.new(illum_mapping_node.outputs["Vector"], ambient_hdri_node.inputs["Vector"])
    bpy.context.scene.world.node_tree.links.new(
        ambient_hdri_node.outputs["Color"],
        ambient_node.inputs["Color"]
    )

    # Load the HDRI image
    ambient_hdri_node.image = bpy.data.images.load(hdri_filepath, check_existing=True)

    # Set the strength of the ambient node
    ambient_node.inputs["Strength"].default_value = strength

    # Set the rotation of the HDRI
    illum_mapping_node.inputs["Rotation"].default_value = hdri_rotation

def set_ambient_light_hdri(hdri_path):
    """Set an HDRI as the ambient light for the world background."""
    ...
    # Set the strength of the background light
...
def get_direction(A, B):
    ...
    direction = ""
    if transformed_vector[0] > 0:
        direction += "front "
    elif transformed_vector[0] < 0:
        direction += "back "
    
    if transformed_vector[1] > 0:
        direction += "right"
    elif transformed_vector[1] < 0:
        direction += "left"
